...
import { submitScale } from '@/server/scales'
...
/**
 * The patient sends a questionnaire. Same shape as `submitIntakeAction`: the
 * token is bound by the page, the scale is read from the link on the server,
 * and a missing answer comes back with the others still ticked.
 */
export async function submitScaleAction(
  token: string,
  _previous: FormState,
  formData: FormData,
): Promise<FormState> {
  ...
  let result
  try {
    result = await submitScale(token, values)
  } catch (error) {
    return formErrorFor(error, 'No pudimos enviar las respuestas. Probá de nuevo en un rato.', values)
  }

  if (result === 'ok') return formOk()
  if (result === 'submitted') return formError('Estas respuestas ya se enviaron. No hace falta hacer nada más.')
  if (result === 'expired') return formError('Este link venció. Pedile uno nuevo a quien te lo mandó.', values)
  return formError('Este link no es válido. Pedile uno nuevo a quien te lo mandó.', values)
}
